...
const EditarContaPage = () => {
    ...
    const fetchConta = useCallback(async () => {
        setLoading(true);
        try {
            const conta: ContaDTO = await ContaService.buscarPorId(id);
            setFormData({
                nome: conta.nome || '',
                site: conta.site || '',
                tipo: conta.tipo || '',
                setor: conta.setor || '',
                telefoneEscritorio: conta.telefoneEscritorio || '',
                email: conta.email || '',
                classificacao: conta.classificacao || '',
                faturamentoAnual: conta.faturamentoAnual,
                funcionarios: conta.funcionarios,
                descricao: conta.descricao || '',
                enderecoCobranca: conta.enderecoCobranca || '',
                cidadeCobranca: conta.cidadeCobranca || '',
                estadoCobranca: conta.estadoCobranca || '',
                cepCobranca: conta.cepCobranca || '',
                paisCobranca: conta.paisCobranca || '',
                enderecoEntrega: conta.enderecoEntrega || '',
                cidadeEntrega: conta.cidadeEntrega || '',
                estadoEntrega: conta.estadoEntrega || '',
                cepEntrega: conta.cepEntrega || '',
                paisEntrega: conta.paisEntrega || ''
            });
        } catch (error) {
            toast.current?.show({
                severity: 'error',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await ContaService.atualizar(id, payload as ContaDTO);
